...
static int set_gpio_state_and_read(struct pinctrl *pinctrl, struct pinctrl_state *state, int gpio_num, int *gpio_value)
{
    ...
    if (ret) {
        TMS_ERR("Failed to select pinctrl state\n");
        return ret;
    }
...
static int get_gpio_value_three(struct device *dev, int *gpio_value)
{
    ...
    TMS_INFO("%s, gpio_num = %d\n", __func__, gpio_num);

    if (!gpio_is_valid(gpio_num)) {
        TMS_ERR("%s, id-gpio is not valid\n", __func__);
        return -EINVAL;
    }

    ret = gpio_request(gpio_num, "id-gpio");
    if (ret < 0) {
        TMS_ERR("%s, failed to request id-gpio, error_code=%d\n", __func__, ret);
        return ret;
    }
    TMS_INFO("%s, id-gpio = %d\n", __func__, gpio_num);
    ret = gpio_direction_input(gpio_num);
    if (ret < 0) {
        TMS_ERR("%s, failed to set id-gpio direction to input, error_code=%d\n", __func__, ret);
        goto free_gpio;
    }

    pinctrl = devm_pinctrl_get(dev);
    if (IS_ERR(pinctrl)) {
	TMS_ERR("pinctrl init fail: %ld\n", PTR_ERR(pinctrl));
	ret = PTR_ERR(pinctrl);
        goto free_gpio;
    }
    ...
    if (IS_ERR(default_state) || IS_ERR(pullup_state) || IS_ERR(pulldown_state)) {
        TMS_ERR("pinctrl state lookup fail: default_state=%ld, pullup_state=%ld, pulldown_state=%ld\n",
    ...
    TMS_INFO("%s, id-gpio value_down is %d", __func__, value_down);
    if (ret) {
        TMS_ERR("Failed to read GPIO in pulldown state\n");
...
restore_default:
    ...
    if (default_ret) {
        TMS_ERR("Failed to select default pinctrl state: default_ret = %d\n", default_ret);
    }
...
static int read_id_properties(struct device_node *np, u32 id_count, struct id_entry *id_entries)
{
    ...
    TMS_INFO("read_id_properties success");
    return 0;
}
...
static int checkNfcChip(struct device *dev)
{
    ...
    else
    {
        if (SUPPORT_MIXED_CHIPSET == mixed_chipset)
        {
            ...
            if (err)
            {
                TMS_ERR("%s error: read_id_properties failed", __func__);
                kfree(id_entries);
                return err;
            }
